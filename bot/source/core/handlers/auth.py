    ...
    try:
        async with (await backend.get(f"/auth/me", headers={"X-Telegram-User-Id": str(telegram_id)})) as response:
            if response.status == 200:
                ...
                await message.answer(t["register.enter_user_name"])
...
async def name(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(t["register.enter_company_name"])
...
async def process_surname(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(t["register.enter_company_info"])
    await state.set_state(RegisterState.company_info)


@router.message(RegisterState.company_info)
